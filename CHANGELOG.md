...
- Der Compose-Healthcheck nutzt jetzt den tenant-aware `solr-tenant.sh healthcheck` statt nur Solr-Liveness zu prüfen. Root-cause: Der alte Check konnte grün melden, obwohl Tenant-Drift oder defekte ACLs noch vorhanden waren.
- Der tenant-aware `healthcheck` behandelt SolrCloud-Drift nicht mehr als Startup-Fehler, sondern nur Bootstrap/Auth-Status. Root-cause: Drift gehört in `drift-detect`; der Healthcheck hat legitime, bereits in ZooKeeper persistierte Zustände sonst fälschlich als Fehler markiert.
### Added
